# Purpose
# Test whether parent comment constructiveness predicts reduced child comment destructiveness.
# This tests cross-propagation: constructiveness suppresses destructiveness.
#
# Reference: Main text line 101
...
dyads_analysis <- parent_child_data %>%
  ...
  filter(!is.na(parent_constructiveness), !is.na(child_destructiveness))
...
# Model: Parent C → Child D (Beta Regression)
cat("=== MODEL: PARENT CONSTRUCTIVENESS → CHILD DESTRUCTIVENESS ===\n\n")
...
cat("\n=== KEY RESULTS ===\n")
cat("Parent Constructiveness → Child Destructiveness:\n")
...
cat("    is associated with reduced child comment destructiveness.\n")
...
saveRDS(results, "analysis/propagation/RQ3_parent_C_to_child_D_results.rds")
