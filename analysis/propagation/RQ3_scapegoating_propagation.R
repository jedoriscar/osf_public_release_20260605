...
# child comment scapegoating (language that blames specific groups).
#
...
child_scapegoating_lookup <- comment_lookup %>%
  select(comment_id, child_prob_scapegoating = prob_scapegoating)

dyads_analysis <- parent_child_data %>%
  left_join(parent_lookup, by = c("parent_comment_id" = "comment_id"), relationship = "many-to-one") %>%
  left_join(child_scapegoating_lookup, by = "comment_id", relationship = "many-to-one") %>%
  filter(!is.na(parent_constructiveness), !is.na(parent_destructiveness))

# Create binary scapegoating indicator (≥0.6 threshold)
dyads_analysis <- dyads_analysis %>%
  mutate(
    child_scapegoating_binary = as.numeric(child_prob_scapegoating >= 0.6)
  ) %>%
  filter(!is.na(child_scapegoating_binary))

# Model: Parent C and D → Child Scapegoating (Logistic MLM)
cat("=== MODEL: PARENT DISCOURSE → CHILD SCAPEGOATING ===\n\n")

mod1 <- glmer(
  child_scapegoating_binary ~ parent_constructiveness + parent_destructiveness + (1|video_id),
  data = dyads_analysis,
  family = binomial,
...
cat("\n=== KEY RESULTS ===\n")
cat("Parent Constructiveness → Child Scapegoating:\n")
...
cat("\nParent Destructiveness → Child Scapegoating:\n")
cat(sprintf("  Log Odds = %.3f (SE = %.3f), z = %.2f, p < .001\n", 
...
saveRDS(results, "analysis/propagation/RQ3_scapegoating_propagation_results.rds")
