# Purpose
# Document Model 7 specification: Parent D → Child D (beta regression).
# Provides complete model specification for propagation analyses.
#
# Reference: SI Appendix Section 4.1, Main text lines 102-103
...
# Model specification
cat("=== MODEL 7: PARENT D → CHILD D (BETA REGRESSION) ===\n\n")
...
cat("Formula:\n")
cat("  child_destructiveness_beta ~ parent_destructiveness + (1|video_id)\n\n")

cat("Fixed Effects:\n")
cat("  - parent_destructiveness: Parent comment destructiveness (continuous, 0-1)\n\n")
...
cat("\n=== KEY RESULTS ===\n")
cat(sprintf("Parent Destructiveness coefficient: β = %.3f (SE = %.3f)\n", coefs[2], ses[2]))
...
results <- list(
  ...
  formula = "child_destructiveness_beta ~ parent_destructiveness + (1|video_id)",
  coefficient = coefs[2],
  se = ses[2],
  ci = ci[2,],
...
saveRDS(results, "analysis/supplement_s4_1_model_specs/SM4.1_model_7_propagation_D_to_D_spec_results.rds")
