# Purpose
# Test whether individual constructive feature (curiosity) predicts engagement outcomes.
...
# Models
mod1_likes <- glmer.nb(like_count ~ curiosity_binary + (1|video_id), data = analysis_data,
                       control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
mod1_replies <- glmer.nb(reply_count ~ curiosity_binary + (1|video_id), data = analysis_data,
                         control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
mod1_alg <- glmer(top_comment_binary ~ curiosity_binary + (1|video_id), data = analysis_data,
...
saveRDS(results, "analysis/supplement_s4_6_feature_specific_models/SM4.6_curiosity_engagement_results.rds")
