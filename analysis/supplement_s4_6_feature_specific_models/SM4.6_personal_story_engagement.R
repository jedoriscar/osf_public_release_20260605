# Purpose
# Test whether individual constructive feature (personal story) predicts engagement outcomes.
...
# Models
mod1_likes <- glmer.nb(like_count ~ personal_story_binary + (1|video_id), data = analysis_data,
                       control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
mod1_replies <- glmer.nb(reply_count ~ personal_story_binary + (1|video_id), data = analysis_data,
                         control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
mod1_alg <- glmer(top_comment_binary ~ personal_story_binary + (1|video_id), data = analysis_data,
...
saveRDS(results, "analysis/supplement_s4_6_feature_specific_models/SM4.6_personal_story_engagement_results.rds")
