# Purpose
# Test whether destructiveness interacts with stance to predict replies.
...
# Filter to YouTube only. Use COMMENT-level engagement: replies (not video-level reply_count).
analysis_data <- analysis_data %>%
  ...
  filter(!is.na(comment_replies), !is.na(divisiveness_raw), stance_label %in% c("Pro-Diversity", "Anti-Diversity"))

# Model: Replies with Stance × Destructiveness Interaction
cat("=== MODEL: REPLIES WITH STANCE × DESTRUCTIVENESS INTERACTION ===\n\n")

mod1 <- glmer.nb(
  comment_replies ~ stance_binary + divisiveness_raw + 
...
# Engagement is COMMENT-level (comment_replies = replies per comment). Stance is COMMENT-level.
coefs <- fixef(mod1)
...
saveRDS(results, "analysis/stance_robustness/ROB2_interaction_replies_destructiveness_results.rds")
